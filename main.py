    ...
    def reply_to_thread(self):
        for i in tuple(reversed(self.threads_topics))[:4]:
            ...
            while True:
                ...
                message = f"{message.replace("Best regards,", "").replace("Respected cs2 community member", "")}[hr][/hr][i]Apply today for an interview to join our exclusive steam community: [url=steamcommunity.com/groups/communityleaders2]CS2 Community Leaders Steam Group[/url]\nBest regards, Respected cs2 community member[/i]"
                data = {
                    "comment":message,
